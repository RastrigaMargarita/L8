let app = {
    ...
    generateBoard() {
        ...
        for (let i = 0; i < 8; i++) {
            ...
            board += row;
            this.config.rowStartWithColor = !this.config.rowStartWithColor;
        }
    ...
    generateRow(rowNum) {
        ...
        for (let i = 0; i < 8; i++) {
            let field = "";
            field = this.generateField(rowNum, this.config.cols[i]);
            row += field;
        }
        return `<tr>${row}</tr>`;
    },
    ...
    generateField(rowNum, colChar) {
        this.config.rowStartWithColor = !this.config.rowStartWithColor;
        return `<td data-rownum="${rowNum}" data-colchar="${colChar}" class="${this.config.rowStartWithColor ? "white" : "black"}"></td>`;
    },
